...
function App() {
  ...
  const [ activeAlgorithms, setActiveAlgorithms ] = useState<Record<AlgorithmName, boolean>>({dp: false, bruteforce: false});
  const [currentSteps, setCurrentSteps] = useState<Record<AlgorithmName, number>>({dp: 0, bruteforce: 0})

  // INPLEMENT USESTATE STATUS, SETSTATUS!!! (refactor, mb replace activeAlgorithms)
  const [status, setStatus] = useState<Record<AlgorithmName, TAlgorithmStatus>>({dp: "unavailable", bruteforce: "unavailable"});
  ...
  const controlAlgorithms = (status: TAlgorithmStatus, param: "all" | AlgorithmName = "all" ): void => {
    switch (param) {
      case "all": runAlgorithms(status); return;
      case "dp": setStatus(prev => ({...prev, dp: status})); return;
      case "bruteforce": setStatus(prev => ({...prev, bruteforce: status})); return;
    }
  }
    ...
    <>
      <AnimatePresence mode="wait">
        <main  className=" w-screen  bg-neutral-800 box-border py-10 h-screen space-y-10  flex flex-col justify-center ">
            <Input
              isAlgosRunning={(Object.values(status) as TAlgorithmStatus[]).some(el => el === "running")}
              isAlgosPaused={(Object.values(status) as TAlgorithmStatus[]).some(el => el === "paused")}
        ...
        <motion.section ref= {boundsRef} className="w-[calc(100%-2rem)] z-1 absolute rounded-2xl p-4 border border-neutral-500 left-4 top-4 h-[calc(100%-2rem)] "></motion.section>
            ...
            {/* ) : ( */}
              {/* <h1 className="text-white absolute bottom-2 left-2 text-4xl ">Still empty...</h1> */}
