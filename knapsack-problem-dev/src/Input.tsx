...
import chooseAlgorithm from "./helpers/chooseAlgorithm";
...
interface InputProps {
  ...
  setCurrentSteps: React.Dispatch<React.SetStateAction<Record<AlgorithmName, number>>>;

  // control the algos - pause and run
  controlAlgorithms: (status: TAlgorithmStatus, param?: "all" | AlgorithmName) => void;
...
export const Input = ({
  ...
  const handleSubmit = (e: SubmitEvent<HTMLFormElement>) => {
    ...
    if (parsedValues && parsedWeights) {
      const updates: Partial<IAlgorithms>= {};
      const steps: Partial<Record<AlgorithmName, number>> = {};

      // run all exsisted algorithms
      (Object.keys(selectedAlgorithms) as AlgorithmName[]).forEach((k) => {
        if (selectedAlgorithms[k] !== true) {
        controlAlgorithms("unavailable", k);
          return;
        }
        const result = chooseAlgorithm(
          k,
          parsedWeights,
          parsedValues,
          parsedCapacity,
        ...
        // set 'run' this selected algo
        controlAlgorithms("running", k);
      });

      setAlgorithms((prev) => ({ ...prev, ...updates }));
      setCurrentSteps((prev) => ({ ...prev, ...steps}));

      // and last - if no one algo has been selected
      if (Object.values(selectedAlgorithms).every((el) => el == false)) {
        setSelectedAlgorithms((prev) => ({ ...prev, dp: true }));

        setCurrentSteps((prev) => ({ ...prev, dp: 0 }));

        // set dp algo active
        const { steps, time } = chooseAlgorithm(
          "dp",
          parsedWeights,
          parsedValues,
          parsedCapacity,
        ...
        setAlgorithms((prev) => ({
          ...prev,
          dp: { steps: steps, time: time },
        }));
        controlAlgorithms("running","dp");
      }
    }
    setIsOpened(false);
  };
  ...
  return (
    ...
    >
      <button
        ...
        className="text-white font-semibold text-2xl transition transform-[rotate(90deg)] absolute left-[1rem] top-4"
        style={{ transform: isOpened ? "rotate(90deg)" : "rotate(270deg)  " }}
      >
        {">"}
      </button>
      <form onSubmit={handleSubmit} className="flex flex-col gap-5">
        ...
        <motion.section
          ...
          className={` ${
            isOpened
              ? " opacity-100"
              : "bottom-200 absolute pointer-none select-none opacity-0"
        ...
        >
          <div className="w-full space-y-4">
            ...
            <div className="flex flex-col gap-1">
              <label className="text-white">Enter weights array:</label>
              <input
                required
                disabled={isAlgosRunning}
                autoComplete="off"
            ...
            <div className="flex flex-col gap-1">
              <label className="text-white">Enter knapsack capacity:</label>
        ...
        <motion.div
          layout
          className={`flex justify-between  h-fit items-center  gap-6  ${isOpened ? "w-full" : "absolute w-[95%] right-0 top-0 h-full px-2  items-center align-top"}`}
        >
          <motion.button
            animate={controls}
            // IF ALGOS RUNNING  - U CANT CLICK ON RANDOMIZER BUTTON
            disabled={isAlgosRunning}
            onClick={() => {
          ...
          <div
            className={` flex-row relative flex justify-center  items-center  space-x-2 ${isOpened ? "static" : "  "}`}
          >
            {isAlgosRunning && (
              <motion.div
                animate={{ rotate: 360 }}
            ...
            {isAlgosRunning && (
              <RunAlgoButton
                handleAlgorithms={() => handleControlSelected("paused")}
              ...
              {isAlgosPaused && (
              <RunAlgoButton
                handleAlgorithms={() => handleControlSelected("running")}
